...
export const GetDepartamentos = async (): Promise<IDepartamentos[]> => {
    const response = await fetch(`${dicionarioDeRotas.api.departamentos}/listar`, {
        method: "GET",
    });
...
export const GetDepartamentoById = async (
    ...
    const response = await fetch(`${dicionarioDeRotas.api.departamentos}/${id}`, {
        method: "GET",
    });
...
export const CadastrarDepartamento = async (departamento: {
    ...
    const response = await fetch(`${dicionarioDeRotas.api.departamentos}/criar`, {
        method: "POST",
        headers: {
...
export const PutDepartamento = async (id: string, departamento: {
    descricao: string;
    codigo: string
}): Promise<IDepartamentos> => {
    const payload = {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Erro ao atualizar o departamento (Status ${response.status})`);
    }
